...
if __name__ == "__main__":
    ...
    cls_weights = np.array([1, 10], np.float32)
    # ------------------------------------------------------------------#
    ...
    if distributed:
        ...
        # Default to cuda:1 since usually cuda:0 is adopted as the monitor output
        # If there is only 1 GPU, set cuda:0 here instead
    ...
    if local_rank == 0:
        ...
        # loss_history = LossHistory(tb_dir, model, input_shape=input_shape)
    else:
        loss_history = None
    ...
    if Cuda:
        if distributed:
            ...
            # Ubuntu 18.04 will throw an error since our model only takes the first half of batch
    ...
    if True:
        ...
        optimizer = {
            ...
            'sgd': optim.SGD([{'params': model.parameters(), 'lr': Init_lr_fit, 'momentum': momentum, 'nesterov': True, 'weight_decay': weight_decay},
                              {'params': awl.parameters()}], Init_lr_fit, momentum=momentum, nesterov=True,
        ...
        # train_2019_dataset = DeeplabDataset(input_shape, num_classes, True, dataset_2019_path)
        # train_2022_dataset = DeeplabDataset(input_shape, num_classes, True, dataset_2022_path)
        # dataset = ConcatDataset([train_2019_dataset, train_2022_dataset])
        dataset = DeeplabDataset(input_shape, num_classes, True, dataset_2019_path, dataset_2022_path)
        ...
        #   记录eval的map曲线
        # ----------------------#
        # img_path_22 = os.path.join(dataset_2022_path, 'images')
        # img_list_22 = os.listdir(img_path_22)
        # img_path_19 = os.path.join(dataset_2019_path, 'images')
        # img_list_19 = os.listdir(img_path_19)
        # img_list = img_list_19 + img_list_22
        # val_list = val_dataset.get_img_list()
        val_list = [[item[4], item[5]] for item in val_dataset]
        val_list = list(chain(*val_list))
        if local_rank == 0:
            eval_callback = EvalCallback(model, input_shape, num_classes, val_list, dataset_2021_path, log_dir, Cuda, eval_flag=eval_flag, period=eval_period)
        else:
            eval_callback = None
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            # 这里p理论上应该用source_dataset的length作为参数
            # p = float(epoch*len(train_dataset))/UnFreeze_Epoch/len(train_dataset)
            # alpha = 2./(1.+np.exp(-10*p))-1
            alpha = 1.  # by default
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch,
                          epoch_step, epoch_step_val, gen, gen_val, Freeze_Epoch, UnFreeze_Epoch, Cuda, dice_loss, focal_loss, tversky_loss,
                          cls_weights, num_classes, fp16, scaler, save_period, save_dir, log_dir, alpha, awl, local_rank)
            if distributed:
                dist.barrier()
