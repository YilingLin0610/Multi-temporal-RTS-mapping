...
def Focal_Loss(inputs, target, cls_weights, num_classes=21, alpha=0.25, gamma=2):
    n, c, h, w = inputs.size()
    nt, ht, wt = target.size()
    if h != ht and w != wt:
        inputs = F.interpolate(inputs, size=(ht, wt), mode="bilinear", align_corners=True)
...
def Focal_Tversky_loss(inputs, target, alpha=0.7, beta=0.3, gamma=0.75, smooth=1e-5):
    # gamma varies in range [1, 3]
    tversky_loss = Tversky_loss(inputs, target, alpha, beta, smooth)
    focal_tversky_loss = torch.pow(tversky_loss, gamma)
